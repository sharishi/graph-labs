...
start_point = 0
t = None
from graph_ostov_prima import matrix_filling, generate_matrix
dict_list = None
current_vertex = 0
...
def display_data():
    ...
    matrix_label.pack()
    sequence_label = tk.Label(data_window, text="Sequence Data: " + " ".join(map(str,ostov_turple)))
    print(ostov_turple)
    sequence_label.pack()
...
##############################алгоритм##############################
    ...
    def get_weight(ribs_data):
        result_list = []
        for rib in ribs_data:
            new_rib = rib + (random.randint(1, 10),)
            result_list.append(new_rib)
        return result_list

    def get_min(info_list, linked_vertices):
        rib_min = (-1, -1, math.inf)  # дефолтное ребро
        for v in linked_vertices:
            for x in info_list:
                if (x[0] == v or x[1] == v) and (x[0] not in linked_vertices or x[1] not in linked_vertices):
                    if x[2] < rib_min[2]:
                        rib_min = x
        return rib_min
    def ostov_implement(rib_data, start_point, node_count):
        try:
            dict_of_weight = get_weight(rib_data)
            clonedList = dict_of_weight.copy()
            linked_vertices = {start_point}  # множество соединенных вершин
            ostov_list = []  # список ребер остова

            while len(linked_vertices) < node_count:
                min_rib = get_min(clonedList, linked_vertices)  # ребро с минимальным весом
                if min_rib[2] == math.inf:  # если ребер нет, то остов построен
                    break

                ostov_list.append(min_rib)  # добавляем ребро в остов
                linked_vertices.add(min_rib[0])  # добавляем вершины в множество U
                linked_vertices.add(min_rib[1])
                clonedList.remove(min_rib)

            print("graph_ostov", ostov_list)
            return ostov_list, dict_of_weight
    ...
    def draw_edges(adjacency_matrix, dict_value, dict_wage):
        ...
        for i in range(1, num_vertices + 1):
            ...
            for j in range(i + 1, num_vertices + 1):
                if adjacency_matrix[i - 1][j - 1] == 1:
                    ...
                    # Поиск соответствующего ребра в ostov_turple
                    for edge in dict_wage:
                        if (edge[0] == i and edge[1] == j) or (edge[0] == j and edge[1] == i):
                            weight = edge[2]
                            # Рисование цифры над ребром\
                            t.pencolor("red")
                            t.penup()
                            t.goto((x1 + x2) / 2,
                                   (y1 + y2) / 2 + 20)  # Перемещаемся в середину ребра и поднимаемся выше
                            t.pendown()
                            t.write(str(weight), align="center", font=("Arial", 14, "normal"))
                            t.pencolor("green")
    ...
    def move_to_next_vertex():
        global current_vertex
        for v in ostov_turple:
            v = ostov_turple[current_vertex]
            first_vertex, second_vertex = v[0], v[1]
            if first_vertex in dict_links and second_vertex in dict_links:
                x1, y1 = dict_links[first_vertex]  # Получаем координаты первой вершины из словаря
                x2, y2 = dict_links[second_vertex]  # Получаем координаты второй вершины из словаря
                ...
                messagebox.showwarning("Warning", f"Vertex {first_vertex} or {second_vertex} is missing coordinates.")
    ...
    global ostov_turple
    ostov_turple, wages_dict = ostov_implement(ribs_data, start_point, nodes_count)
    dict_links = draw_vertices(len(adjacency_matrix))
    draw_edges(adjacency_matrix, dict_links, wages_dict)
    move_to_next_vertex()
